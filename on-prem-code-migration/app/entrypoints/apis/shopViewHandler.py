...
@router.get("/")
async def get_shop_view(shopview: shopView = Depends()):
    ...
    response = msil_iot_psm_get_shop_view.handler(shop_id, **query_params)
    return returnJsonResponse(response)

@router.get("/report")
async def get_shop_view_report(reportView: shopViewReport = Depends()):
    shop_id = reportView.shop_id
    if not shop_id:
        raise HTTPException(status_code=400, detail="Missing 'shop_id' query parameter")
    
    query_params = reportView.model_dump(exclude={"shop_id"}, exclude_none=True)
    response = msil_iot_psm_get_shop_view_report.handler(shop_id, **query_params)
...
@router.get("/graph")
async def get_shop_view_graph(shopviewgraph: shopViewGraph = Depends()):
    shop_id = shopviewgraph.shop_id
    if not shop_id:
        raise HTTPException(status_code=400, detail="Missing 'shop_id' query parameter")

    query_params = shopviewgraph.model_dump(exclude={"shop_id"}, exclude_none=True)
    response = msil_iot_psm_get_shop_view_graph.handler(shop_id, **query_params)
    return returnJsonResponse(response)

@router.get("/uniquecounts")
async def get_uniquePartscounts(uniqueparts: uniquePartsCount = Depends()):
    shop_id = uniqueparts.shop_id
    if not shop_id:
        raise HTTPException(status_code=400, detail="Missing 'shop_id' query parameter")
    
    response = msil_iot_psm_unique_parts_count.handler(shop_id)
    return returnJsonResponse(response)

@router.get("/topbreakdown")
async def get_topbreakdown(topbreakdown: topBreakDown = Depends()):
    shop_id = topbreakdown.shop_id
    if not shop_id:
        raise HTTPException(status_code=400, detail="Missing 'shop_id' query parameter")

    query_params = topbreakdown.model_dump(exclude={"shop_id"}, exclude_none=True)
    response = msil_iot_psm_top_downtime_reasons.handler(shop_id, **query_params)
    return returnJsonResponse(response)

@router.get("/machine")
async def get_machines(machineview: machineView = Depends()):
    shop_id = machineview.shop_id
    if not shop_id:
        raise HTTPException(status_code=400, detail="Missing 'shop_id' query parameter")
    
    query_params = machineview.model_dump(exclude={"shop_id"}, exclude_none=True)
    response = msil_iot_psm_get_machine_view.handler(shop_id, **query_params)
    return returnJsonResponse(response)

@router.get("/machine-trend")
async def get_machine_trend(machineviewgraph: machineViewGraph = Depends()):
    shop_id = machineviewgraph.shop_id
    if not shop_id:
        raise HTTPException(status_code=400, detail="Missing 'shop_id' query parameter")
    
    query_params = machineviewgraph.model_dump(exclude={"shop_id"}, exclude_none=True)
    response = msil_iot_psm_get_machine_trend_graph.handler(shop_id, **query_params)
    return returnJsonResponse(response)