...
class MSILEquipmentRepository(Repository):
    ...
    def get_shop_view(self, shop_id, start_time, end_time, name=None, group=None, view="DAY",material_code=None):
        ...
        with self.session:
            ...
            start_time = ist_tz.localize(datetime.datetime.combine(start_date,start)).replace(tzinfo=None)
            ...
            if end < start:
                end_time = ist_tz.localize(datetime.datetime.combine(end_date,end)).replace(minute=30,second=0,microsecond=0,tzinfo=None)
            else:
                end_time = ist_tz.localize(datetime.datetime.combine(cur_date,end)).replace(minute=30,second=0,microsecond=0,tzinfo=None)
            ...
            if greater_than_start:
                breaks= self.session.query(MSILShopBreaks).filter(MSILShopBreaks.shop_id==shop_id, MSILShopBreaks.start_time<cur_time, MSILShopBreaks.start_time>start)
                for item in breaks:
                    if item.end_time> cur_time:
                        break_duration += (cur_datetime- ist_tz.localize(datetime.datetime.combine(cur_date,item.start_time)).replace(tzinfo=None)).total_seconds()/60
                    else:
                        break_duration += item.duration_minutes
            else:
                breaks = self.session.query(MSILShopBreaks).filter(MSILShopBreaks.shop_id==shop_id, or_(MSILShopBreaks.start_time>start,MSILShopBreaks.start_time<cur_time))
                for item in breaks:
            ...
            all_breaks = self.session.query(MSILShopBreaks).filter(MSILShopBreaks.shop_id==shop_id)
            total_breaks = 0
            if start_time is not None and end_time is not None:
                ...
                runtime = (end_time - start_time).total_seconds()/3600 - total_breaks/60
            else:
            ...
            if view=="DAY":
                ...
                if material_code:
                    machine_telemetry_filter.append(MSILMachineTelemetry.material_code == material_code)
                telemetry_subquery =  self.session.query(MSILMachineTelemetry.equipment_id,
                                                         func.sum(MSILMachineTelemetry.quantity).label("temp_quantity"),
                ...
                downtime_filter = []
                if material_code:
                    downtime_filter.append(MSILDowntime.material_code == material_code)
            ...
            query = query.add_column(
                    case(
                        ...
                        else_=(func.coalesce(actual_subquery.c.actual, 0) * 100.0) / func.coalesce(plan_subquery.c.target, 0)
                    ).label('target_percent')
)     
            query = query.add_column(get_efficiency_by_shop_id(shop_id,downtime_subquery.c.breakdown,downtime_subquery.c.idle,runtime,downtime_subquery.c.scheduled,actual_prod=column('actual'),planned_prod=(func.coalesce(plan_subquery.c.target, 0))).label('efficiency'))
            query = query.add_column(case(((((runtime*60 - func.coalesce(downtime_subquery.c.breakdown,0)) / (runtime*60 - func.coalesce(downtime_subquery.c.idle,0))) * 100) < self.model_type.efficiency, 1), else_=0).label('critical'))
            query = query.add_column(get_sph_by_shop_id(shop_id,column('actual'),runtime,downtime_subquery.c.idle,downtime_subquery.c.breakdown,downtime_subquery.c.scheduled).label('sph'))
        ...
        # Included scheduled downtime
            ...
            query = query.add_column(
                    case(
                        ...
                        else_=(func.coalesce(actual_subquery.c.actual, 0) * 100.0) / func.coalesce(plan_subquery.c.day_target, 0)
                    ).label('target_percent')
)     
            query = query.add_column(get_efficiency_by_shop_id(shop_id,downtime_subquery.c.breakdown,downtime_subquery.c.idle,runtime,downtime_subquery.c.scheduled).label('efficiency'))
            query = query.add_column(get_sph_by_shop_id(shop_id,column('no_of_stroke'),runtime,downtime_subquery.c.idle,downtime_subquery.c.breakdown,downtime_subquery.c.scheduled).label('sph'))
            query = query.add_column(actual_subquery.c.actual)
            query = query.add_column(column('actual'))
    ...
    def actual_graph(self, shop_id, start_time, end_time, name=None):
        ...
        # start_query = self.session.query(MSILShift).filter(MSILShift.shop_id==shop_id).order_by(MSILShift.shift_name).first()
        start = start_time
        start_time = ist_tz.localize(datetime.datetime.combine(cur_date,start)).replace(tzinfo=None)

        # end_query = self.session.query(MSILShift).filter(MSILShift.shop_id==shop_id).order_by(MSILShift.shift_name.desc()).first()
        next_date = cur_date + datetime.timedelta(days=1)
        end = end_time
        if end < start:
            end_time = ist_tz.localize(datetime.datetime.combine(next_date,end)).replace(tzinfo=None)
        else:
            end_time = ist_tz.localize(datetime.datetime.combine(cur_date,end)).replace(tzinfo=None)

        with self.session:

            subquery = (
                self.session.query(func.generate_series(start_time, cur_datetime, text("'1 minute'")).label('minute')).subquery()
                )
            ...
            telemetry_data ={item[0].strftime("%Y-%m-%d %H:%M"):item[1] for item in telemetry_query}
            while current_timestamp < cur_datetime:
                timestamps_and_quantities.append({'timestamp': current_timestamp.strftime('%Y-%m-%d %H:%M:00'), 'quantity': round(current_quantity,2)})
                current_quantity+=telemetry_data.get(current_timestamp.strftime("%Y-%m-%d %H:%M"),0)
    ...
    def target_graph(self, shop_id, start_time, end_time, name=None, view="DAY"):
        ...
        if cur_time < start:
            ...
            start_date = cur_date
            end_date = cur_date + datetime.timedelta(days=1)
        start_time = ist_tz.localize(datetime.datetime.combine(start_date,start))

        # end_query = self.session.query(MSILShift).filter(MSILShift.shop_id==shop_id).order_by(MSILShift.shift_name.desc()).first()

        end = end_time
        if end < start:
            end_time = ist_tz.localize(datetime.datetime.combine(end_date,end)).replace(minute=30,second=0,microsecond=0)
        else:
            end_time = ist_tz.localize(datetime.datetime.combine(cur_date,end)).replace(minute=30,second=0,microsecond=0)
        ...
        if greater_than_start:
            ...
            for item in breaks:
                if item.end_time> cur_time:
                    break_duration += (cur_datetime- ist_tz.localize(datetime.datetime.combine(cur_date,item.start_time))).total_seconds()/60
                else:
                    break_duration += item.duration_minutes
        else:
            breaks = self.session.query(MSILShopBreaks).filter(MSILShopBreaks.shop_id==shop_id, or_(MSILShopBreaks.start_time>start,MSILShopBreaks.start_time<cur_time))
        ...
        downtime = self.session.query(MSILShopBreaks).filter(MSILShopBreaks.shop_id==shop_id)
        ...
        total_downtime_duration = sum(item.duration_minutes for item in downtime)
        ...
        for dt in downtime:
            if dt.start_time < start:
                break_start = ist_tz.localize(datetime.datetime.combine(end_date,dt.start_time))
            else:
                break_start = ist_tz.localize(datetime.datetime.combine(start_date,dt.start_time))
            if dt.end_time < start:
                break_end = ist_tz.localize(datetime.datetime.combine(end_date,dt.end_time))
            else:
                break_end = ist_tz.localize(datetime.datetime.combine(start_date,dt.end_time))
            downtime_intervals.append((break_start,break_end))

        # return remaining_working_hours

        rate_per_minute = (total_planned_quantity / remaining_working_hours) / 60
        ...
        while current_timestamp < time_limit:
            if current_quantity < total_planned_quantity:
                timestamps_and_quantities.append({'timestamp': current_timestamp.strftime('%Y-%m-%d %H:%M:00'), 'quantity': round(current_quantity,0)})
    ...
    def equipment_ids(self,shop_id):
        equipment_dict = {}
        with self.session:
            equipments = self.session.query(MSILEquipment).filter(MSILEquipment.shop_id==shop_id).all()
    ...
    def check_for_machines(self,machine_names,shop_id):
        with self.session:
            result_dict ={}
            results = self.session.query(self.model_type.name, self.model_type.id).filter(self.model_type.shop_id==shop_id).where(self.model_type.name.in_(machine_names)).distinct().all()
            for item in results:
    ...
    def soft_delete_equipments(self, eqp_ids, shop_id):
        with self.session:
            to_be_deleted = self.session.query(self.model_type).filter(self.model_type.id.in_(eqp_ids),self.model_type.shop_id==shop_id).all()
    ...
    def get_equipment_by_id(self, equipment_id):
        with self.session:
            query = self.session.query(MSILEquipment).filter(MSILEquipment.id == equipment_id)
        ...
        with self.session:
            ...
            telemetry_query = self.session.query((func.date_trunc('minutes', MSILMachineTelemetry.timestamp)).label('minute_val'), (func.sum(MSILMachineTelemetry.quantity)).label('quantity')) \
                    ...
                    .filter(MSILEquipment.shop_id == shop_id,
                            MSILMachineTelemetry.timestamp > start_time) 
            if name != None:
                telemetry_query = telemetry_query.filter(MSILEquipment.name == name)
    ...
    def get_machine_actual_plan_stroke_breakdown_and_die_change_data(self,shop_id,start_time,end_time,machine_id=None):
        ...
        with self.session:
            ...
            if machine_id != None:
                plan_query = plan_query.filter(MSILPlan.equipment_id == machine_id)
            ...
            if machine_id != None:
                actual_query = actual_query.filter(MSILMachineTelemetry.equipment_id == machine_id)
            ...
            if machine_id != None:
                stroke_subquery = stroke_subquery.filter(MSILMachineTelemetry.equipment_id == machine_id)

            total_stroke =  self.session.query(func.max(stroke_subquery.c.station_quantity).label("total_stroke"))
