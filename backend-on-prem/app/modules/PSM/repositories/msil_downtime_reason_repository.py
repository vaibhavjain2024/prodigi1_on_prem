...
class MSILDowntimeReasonRepository(Repository):
        ...
        with self.session:
            for item in combination_list:
                result = self.session.query(self.model_type).filter(and_(self.model_type.shop_id == shop_id,
                                                                    self.model_type.tag == str(
                                                                        item[2]),
                                                                    self.model_type.reason_type == str(
                                                                        item[1]).strip().lower(),
                                                                    self.model_type.reason == str(
                                                                        item[0])
                                                                    )).first()
                if not result:
    ...
    def id_by_combination(self, shop_id):
        ...
        with self.session:
            ...
            for item in result:
                reason_dict[(item.reason, item.reason_type,
                             item.tag)] = item.id
            return reason_dict

    def delete_reason_tags(self, shop_id, reason_ids):
        with self.session:
            self.session.query(self.model_type).filter(self.model_type.shop_id == shop_id,
                                                       self.model_type.id.in_(reason_ids)).update({self.model_type.tag: None})
            self.session.commit()
            return True
