...
class MSILDowntimeRemarkRepository(Repository):
    ...
    def get_remark_list(self, shop_id):
        try:
            ...
            query = (
                self.session.query(
                    MSILDowntimeReasons.id, MSILDowntimeReasons.reason, MSILDowntimeRemarks.remark)
                .join(MSILDowntimeReasons, self.model_type.reason_id.cast(Integer) == MSILDowntimeReasons.id)
    ...
    def get_remarks_by_reason_id(self, reason_id):
        with self.session:
            try:
                query = (
                    self.session.query(MSILDowntimeRemarks.remark)
                    .filter(MSILDowntimeRemarks.reason_id == reason_id)
                    .all()
                )
                remarks = [row.remark for row in query]
                return remarks
            except Exception as e:
                raise e
